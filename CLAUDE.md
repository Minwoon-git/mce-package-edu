...
**위임 규칙:**
- 🔑 **객체 이름 접두어를 모든 워커 프롬프트에 명시한다.** 이 프로젝트가 만드는 DE·저니·Automation·Query·이메일 에셋·Event Definition은 전부 `<접두어>_` 로 시작한다(기본값 = 고객사명, 사용자가 "내 접두어는 EDU01" 처럼 선언하면 그 값). 상위가 STEP 0(또는 첫 생성 작업) 시작 시 접두어를 확정하고, 이후 **모든 하위 호출 프롬프트에 접두어를 적어 넘긴다** — 워커는 격리 실행이라 사용자 선언을 직접 보지 못하므로, 전달하지 않으면 접두어 없는 이름으로 만든다. 같은 BU를 여러 사람/고객사가 공유할 때 **이것이 유일한 격리 수단**이다. (규칙 상세: [`reference/schema-mapping.md`](.claude/skills/mce-campaign/reference/schema-mapping.md) "🔑 `<접두어>`")
...
|---|---|---|---|---|
| [`mce-schema-agent`](.claude/agents/mce-schema-agent.md) | STEP 0 | (A)스키마 파일 분석→원본 컬럼 개념 태깅·조인키+HITL 목록 / (B)빈 RAW DE(원본 컬럼명)·활성 고객사 가이드 MD 자동생성·CSV 업로드 안내 | (A)스키마 파일 경로/내용 / (B)확정 매핑+HITL 확정값+고객사명 | (A)매핑표+확인목록 / (B)RAW DE 목록·업로드 안내·가이드 MD 경로 |
